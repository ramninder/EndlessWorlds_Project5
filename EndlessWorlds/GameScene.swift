...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    @objc let swipeRightRec = UISwipeGestureRecognizer()
    
    @objc let swipeUpRec = UISwipeGestureRecognizer()
    @objc let swipeDownRec = UISwipeGestureRecognizer()
    
    
    @objc var levelUnitCounter:CGFloat = 0
    @objc var levelUnitWidth:CGFloat = 0
    @objc var levelUnitHeight:CGFloat = 0
    @objc var initialUnits:Int = 2

    @objc var screenWidth:CGFloat = 0
    @objc var screenHeight:CGFloat = 0
    @objc let worldNode:SKNode = SKNode()
    @objc let thePlayer:Player = Player(imageNamed: "ogre_run1")
    
    @objc let loopingBG:SKSpriteNode = SKSpriteNode(imageNamed: "Looping_BG")
    @objc let loopingBG2:SKSpriteNode = SKSpriteNode(imageNamed: "Looping_BG")
   
    
    @objc var levelUnitCurrentlyOn:LevelUnit?
    
    @objc var isDead:Bool = false
   
    @objc var onPlatform:Bool = false
    @objc var currentPlatform:SKSpriteNode?
    
    
    @objc let startingPosition:CGPoint = CGPoint(x: 50, y: 0)
  ...
  @objc  override func didMove(to view: SKView) {
   ...
   @objc override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // physicsBody?.applyForce(CGVector(dx:1, dy:5))
        
       
    }
    
    
    @objc func startLoopingBackground(){
    ...
    @objc func centerOnNode(_ node:SKNode) {
    ...
    @objc func resetLoopingBackground(){
